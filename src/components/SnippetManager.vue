    ...
    <div class="w-full max-w-[760px] bg-white dark:bg-gray-800 dark:text-gray-100 rounded-lg shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col max-h-[80vh]"
      ...
      <div class="flex items-center justify-between px-4 py-2.5 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
        <div class="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-200">
          ...
          <span>代码片段</span>
          <span class="text-xs text-gray-400">· 输入前缀后按 Tab 展开（{{ '$0' }} 为光标落点）</span>
        </div>
          ...
          <div v-for="s in snippets" :key="s.id" class="px-4 py-2 flex items-start gap-3 hover:bg-gray-50 dark:hover:bg-gray-900/40">
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2">
                ...
                <span class="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400">{{ s.language && s.language !== '*' ? s.language : '所有语言' }}</span>
            ...
            <div class="flex items-center gap-1 flex-shrink-0">
              <button class="p-1 rounded text-gray-400 hover:text-blue-500 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer" title="编辑" @click="startEdit(s)">
                <Pencil class="w-3.5 h-3.5"/>
              </button>
              <button class="p-1 rounded text-gray-400 hover:text-red-500 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer" title="删除" @click="remove(s.id)">
                <Trash2 class="w-3.5 h-3.5"/>
              </button>
            </div>
          </div>
        </div>
        <div v-else class="px-4 py-8 text-center text-sm text-gray-400">还没有代码片段，在下方添加</div>
      </div>

      <!-- 编辑/新增表单 -->
      <div class="border-t border-gray-200 dark:border-gray-700 p-3 flex-shrink-0 space-y-2">
        <div class="flex gap-2">
          <input v-model="form.prefix" placeholder="前缀（如 fn）" class="w-40 text-sm border border-gray-300 dark:border-gray-600 dark:bg-gray-900 rounded px-2 py-1 focus:outline-none focus:border-blue-500"/>
          <input v-model="form.language" placeholder="语言（留空=所有）" class="w-44 text-sm border border-gray-300 dark:border-gray-600 dark:bg-gray-900 rounded px-2 py-1 focus:outline-none focus:border-blue-500"/>
          <input v-model="form.description" placeholder="描述（可选）" class="flex-1 text-sm border border-gray-300 dark:border-gray-600 dark:bg-gray-900 rounded px-2 py-1 focus:outline-none focus:border-blue-500"/>
        </div>
        <textarea v-model="form.body" rows="4" placeholder="片段内容，$0 为展开后光标位置" class="w-full text-sm font-mono border border-gray-300 dark:border-gray-600 dark:bg-gray-900 rounded px-2 py-1.5 resize-none focus:outline-none focus:border-blue-500"/>
        <div class="flex items-center gap-2">
          <Button size="sm" :disabled="!canSave" @click="submit">{{ editingId ? '保存修改' : '添加片段' }}</Button>
          <Button v-if="editingId" size="sm" type="secondary" @click="resetForm">取消编辑</Button>
        </div>
      </div>
    </div>
  </div>
