<template>
  <div class="flex flex-col h-full bg-white dark:bg-gray-900">
    ...
    <div v-else class="flex-1 overflow-auto p-2 font-mono text-xs">
      <div v-if="!stable.trim()" class="text-gray-400 px-2 py-4 text-center">{{ t('view.emptyYaml') }}</div>
...
const parseResult = computed<{ value: any; error: string }>(() => {
  ...
  try {
    ...
    return {value: undefined, error: t('view.yamlParseFail') + (e?.reason || e?.message || e)}
  }
})
